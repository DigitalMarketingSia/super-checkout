...
export default function InstallerWizard() {
    ...
    const handleLicenseSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (data.valid) {
                addLog('Licen├ºa validada com sucesso!');
                setTimeout(() => {
                    setLoading(false);
                    runSuccessAnim('Licen├ºa Validada!', () => {
                ...
                throw new Error(data.message || 'Licen├ºa inv├ílida');
            }

        } catch (error: any) {
            console.error(error);
            addLog(`Erro: ${error.message} `);
            showAlert('Erro de Licen├ºa', error.message, 'error');
            setLoading(false);
        }
    };
    ...
    const handleSupabaseManualSubmit = () => {
        if (!supabaseUrl) return setError('URL ├® obrigat├│ria');
    ...
    const handleSupabaseCallback = async (code: string) => {
        ...
        try {
            ...
            addLog('Ô£à Projeto Supabase criado com sucesso!');
            setIsConnectingSupabase(false);
            runSuccessAnim('Conex├úo Estabelecida!', () => {
                setCurrentStep('supabase_migrations');
            });
            // Go to migrations
        } catch (error: any) {
    ...
    const handleKeysSubmit = (e: React.FormEvent) => {
        ...
        if (!anonKey || !serviceKey) {
            showAlert('Campos Obrigat├│rios', 'Por favor, preencha ambas as chaves.', 'error');
            return;
        }
    ...
    const handleDeploySubmit = (e: React.FormEvent) => {
        e.preventDefault();
        if (!vercelDomain) {
            showAlert('Campo Obrigat├│rio', 'Por favor, cole o dom├¡nio (URL) do seu projeto na Vercel.', 'error');
            return;
        }
        let cleanDomain = vercelDomain.replace('https://', '').replace('http://', '').split('/')[0];
        localStorage.setItem('installer_vercel_domain', cleanDomain);
        runSuccessAnim('Dom├¡nio Configurado!', () => {
            ...
            <div className="container mx-auto px-4 py-12 relative z-10 max-w-4xl">
                ...
                <div className="max-w-xl mx-auto mb-16 relative">
                    ...
                    <div className="flex justify-between mt-3 text-xs font-medium text-gray-500 uppercase tracking-wider">
                        ...
                        <span>Configura├º├úo</span>
                        <span>Conclus├úo</span>
                    </div>
                </div>
                ...
                <div className="max-w-2xl mx-auto">
                    ...
                    {currentStep === 'check_subscription' && (
                        <div className="glass-panel border border-white/10 bg-white/5 backdrop-blur-xl rounded-2xl p-8 shadow-2xl animate-in fade-in slide-in-from-bottom-4">
                            ...
                            <h1 className="text-2xl font-bold mb-2 text-white">Validar Licen├ºa</h1>
                            <p className="text-gray-400 mb-6">Digite sua chave de licen├ºa para validar o uso.</p>

                            <form onSubmit={handleLicenseSubmit} className="space-y-4">
                                <div>
                                    <label className="block text-sm font-medium text-gray-300 mb-1.5">Chave da Licen├ºa</label>
                                ...
                                <div className="space-y-4 animate-in fade-in">
                                    <div className="p-4 bg-[#3ECF8E]/10 border border-[#3ECF8E]/20 rounded-xl">
                                        ...
                                        <p className="text-sm text-gray-300">
                                            Vamos criar o projeto e configurar tudo para voc├¬ automaticamente.
                                        </p>
                                    </div>
                                    <button
                    ...
                    {currentStep === 'supabase_migrations' && (
                        <div className="glass-panel border border-white/10 bg-white/5 backdrop-blur-xl rounded-2xl p-8 shadow-2xl animate-in fade-in slide-in-from-bottom-4">
                            ...
                            <div className="space-y-4">
                                ...
                                >
                                    J├í executei o SQL <ChevronRight className="w-4 h-4" />
                                </button>
                            </div>
                        </div>
                    )}
                        ...
                        <div className="glass-panel border border-white/10 bg-white/5 backdrop-blur-xl rounded-2xl p-8 shadow-2xl animate-in fade-in slide-in-from-bottom-4">
                            ...
                            <form onSubmit={async (e) => {
                                ...
                                try {
                                    ...
                                    console.log('­ƒöä Reloading schema cache...');
                                    ...
                                    if (error) {
                                        console.warn('ÔÜá´©Å exec_sql not available, trying alternative method...');
                                        // Alternative: Make a dummy query to trigger cache refresh
                                        await tempClient.from('contents').select('id').limit(0);
                                    }

                                    console.log('Ô£à Schema cache reloaded successfully');
                                } catch (error) {
                                    console.warn('ÔÜá´©Å Could not reload schema cache automatically:', error);
                                }
                        ...
                        <div className="glass-panel border border-white/10 bg-white/5 backdrop-blur-xl rounded-2xl p-8 shadow-2xl animate-in fade-in slide-in-from-bottom-4">
                            ...
                            <div className="space-y-6">
                                ...
                                <button onClick={() => setCurrentStep('vercel_config')} className="w-full bg-gray-800 hover:bg-gray-700 text-white font-bold py-3 rounded-xl flex items-center justify-center gap-2">
                                    J├í fiz o Deploy, quero configurar <ChevronRight className="w-4 h-4" />
                                </button>
                            </div>
                        </div>
                    )}
                    ...
                    {currentStep === 'vercel_config' && (
                        <div className="glass-panel border border-white/10 bg-white/5 backdrop-blur-xl rounded-2xl p-8 shadow-2xl animate-in fade-in slide-in-from-bottom-4">
                            ...
                            <h1 className="text-2xl font-bold mb-2 text-white">Configura├º├úo P├│s-Deploy</h1>
                            <p className="text-gray-400 mb-6">Adicione estas vari├íveis no painel da Vercel para ativar o sistema.</p>

                            <div className="p-4 bg-yellow-500/10 border border-yellow-500/20 rounded-xl mb-8 flex items-start gap-3">
                                <Info className="w-5 h-5 text-yellow-500 shrink-0 mt-0.5" />
                                <p className="text-sm text-yellow-200">
                                    Ap├│s adicionar as vari├íveis abaixo no seu projeto Vercel, voc├¬ precisar├í fazer um <strong>Redeploy</strong> para que elas tenham efeito.
                                </p>
                            </div>
                                        ...
                                        <div>
                                            <h3 className="text-white font-bold flex items-center gap-2">
                                                ...
                                                <a href="https://vercel.com/account/settings" target="_blank" className="text-xs font-normal text-purple-400 hover:text-purple-300 bg-purple-500/10 px-2 py-1 rounded-full flex items-center gap-1 transition-colors">
                                                    Ver Configura├º├Áes <ExternalLink className="w-3 h-3" />
                                                </a>
                                            </h3>
                                        </div>
                                    </div>
                    ...
                    {currentStep === 'success' && (
                        <div className="glass-panel border border-green-500/20 bg-green-500/5 backdrop-blur-xl rounded-2xl p-8 shadow-2xl animate-in fade-in slide-in-from-bottom-4 text-center">
                            ...
                            <h1 className="text-3xl font-bold mb-4 text-white">Instala├º├úo Conclu├¡da!</h1>
                            <p className="text-gray-400 mb-8 max-w-md mx-auto">
                                Parab├®ns! Seu Super Checkout est├í configurado.
                            </p>
