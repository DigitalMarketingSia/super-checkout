...
export const Offers = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      showAlert('Sucesso', 'Oferta criada com sucesso!', 'success');
    } catch (error) {
      console.error('Error creating offer:', error);
      showAlert('Erro', 'Erro ao criar oferta.', 'error');
    }
  };
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      setOffers(await storage.getOffers());
      setDeleteId(null);
      showAlert('Sucesso', 'Oferta excluída com sucesso.', 'success');
    } catch (e) {
      console.error(e);
      showAlert('Erro', 'Erro ao excluir oferta.', 'error');
    } finally {
      setIsDeleting(false);
    }
  };
  ...
  return (
    <Layout>
                ...
                return (
                  <tr key={offer.id} className="hover:bg-gray-50 dark:hover:bg-white/5 transition-colors">
                    ...
                    <td className="px-6 py-4 text-right">
                      <button onClick={() => handleDeleteClick(offer.id)} className="text-gray-400 hover:text-red-500 transition-colors">
      ...
      {/* Create Modal */}
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title="Criar Oferta">
