    ...
    const handleTest = async () => {
        ...
        try {
            ...
            const mockOrder: Order = {
                ...
                payment_method: 'pix',
                items: [{ name: 'Produto Exemplo', price: 197.90, quantity: 1 }],
                created_at: new Date().toISOString()
            };

            // 4. Send the 2 relevant email types
            await Promise.all([
            ...
            alert(`Sucesso! Enviamos 2 e-mails de teste (Pagamento Aprovado e Boleto) para: ${targetEmail}. Verifique sua caixa de entrada e spam.`);
        } catch (error: any) {
            console.error('Test error:', error);
            alert(`Erro ao testar: ${error.message || JSON.stringify(error)}`);
        } finally {
            setTesting(false);
        }
    };
