...
export default async function handler(req: WebhookRequest, res: WebhookResponse) {
    ...
    try {
        ...
        // CRITICAL: Import supabase FIRST to log immediately
        console.log('[Webhook] Loading supabase...');
        const { supabase } = await import('../../services/supabase');
        console.log('[Webhook] Supabase loaded');

        // DEBUG: Immediate Log to DB BEFORE loading paymentService
        try {
            ...
            console.log('[Webhook] Debug log written to DB');
        } catch (logError: any) {
            console.error('[Webhook] Failed to write debug log:', logError.message);
        }
