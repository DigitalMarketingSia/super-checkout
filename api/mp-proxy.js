...
export default async function handler(req) {
    ...
    try {
        ...
        // 3. Health Check
        if (!endpoint) {
            return new Response(JSON.stringify({ status: 'ok', message: 'JS Edge Proxy is running' }), {
                status: 200,
        ...
        console.log(`[JS Proxy] Forwarding ${req.method} to ${targetUrl}`);
        ...
        return new Response(JSON.stringify({ error: error.message || 'Internal JS Proxy Error' }), {
            status: 500,
            headers: { ...corsHeaders, 'Content-Type': 'application/json' }
        });
    }
